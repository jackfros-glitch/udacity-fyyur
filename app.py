...
class Area(db.Model):
  ...
  venues = db.relationship('Venue', backref='areas', lazy=True)
...
class Venue(db.Model):
    ...
    def __repr__(self):
      return f'<Venue ID: {self.id}, Venue name: {self.name}, Venue State: {self.state}>'
...
class Artist(db.Model):
    ...
    def __repr__(self):
      return f'<Artist ID: {self.id}, Artist name: {self.name}, Artist State: {self.state}>'
...
#  Venues
#  ----------------------------------------------------------------
# please would love any feedback of how i could write the code more efficiently from the code reviewer
...
def venues():
  ...
  if not areas:
    ...
    cities = db.session.query(Area).all()
    for v in venues_grouped:
      print(v.city)
      if v not in cities:
        a = Area()
...
def search_venues():
  ...
  result = db.session.query(Venue).filter(Venue.name.like("%"+key_word+"%"))
  count = len(result)
  response = {}
...
def show_venue(venue_id):
  ...
  venue.upcoming_shows_count = upcoming_count
  venue.past_shows = past
  venue.past_shows_count = past_count
...
def search_artists():
  ...
  key_word = request.form.get("search_term")
  result = db.session.query(Artist).filter(Artist.name.ilike("%"+key_word+"%"))
  
  count = db.session.query(Artist).filter(Artist.name.ilike("%"+key_word+"%")).count()
  response ={}
  response['count'] = count
  response['data'] = result
  # response={
...
def show_artist(artist_id):
  ...
  print(artist.upcoming_shows_count, upcoming_count, past_count)
...
def create_show_submission():
  ...
  try:
    ...
    show.start_time =  dateutil.parser.parse(request.form.get('start_time'))
